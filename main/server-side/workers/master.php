...
$master->onMessage = function ($connection, $buffer) use ( $master)
{
    ...
    if ($arr['action'] == "reconn") {
        if ($arr['worker'] == "timer") {
            ...
            echo "\r[" . date('Y-m-d H:i:s') .
                    "][Master][Info] Timer reconnected.          \n";
            echo "\rStatus: Conns: " . $master->active_conn . ", PPS: in: " .
                    $master->pps["in"] . ", out: " . $master->pps["out"] .
                    " Speed: in: " . round($master->speed["in"] / 1024, 3) .
                    "KB/s, out: " . round($master->speed["out"] / 1024, 3) .
                    "KB/s             \r";
        } else {
    ...
    if ($arr['action'] == "new_msg") {
        ...
        if ($arr['handle'] == "in") {
            $master->pps_temp["in"] ++;
            $master->speed_temp["in"] = $master->speed_temp["in"] +
                    $arr["strlen"];
        }
        if ($arr['handle'] == "out") {
            $master->pps_temp["out"] ++;
            $master->speed_temp["out"] = $master->speed_temp["out"] +
                    $arr["strlen"];
        }
    }
    ...
    if ($arr['action'] == "timer") {
        ...
        $master->pps_temp = Array(
                "in" => 0,
                "out" => 0
        );
        $master->speed = $master->speed_temp;
        $master->speed_temp = Array(
                "in" => 0,
                "out" => 0
        );
        echo "\rStatus: Conns: " . $master->active_conn . ", PPS: in: " .

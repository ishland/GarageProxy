...
class Worker
{
     ...
     * @var mixed
     */
    public static $logFile = './workerman.log';
    ...
    protected static function displayUI()
    {
        ...
        foreach (static::$_workers as $worker) {
            static::safeEcho(str_pad($worker->user, static::$_maxUserNameLength + 2) . str_pad($worker->name, static::$_maxWorkerNameLength + 2) . str_pad($worker->getSocketName(), static::$_maxSocketNameLength + 2) . str_pad(' ' . $worker->count, 9) . " \033[32;40m [OK] \033[0m\n");
        }
        static::safeEcho("----------------------------------------------------------------\n");
        if (static::$daemonize) {
            global $argv;
    ...
    protected static function reinstallSignal()
    {
        ...
        static::$globalEvent->add(SIGINT, EventInterface::EV_SIGNAL, array(
            '\Workerman\Worker',
            'signalHandler'
        ));
        // reinstall graceful stop signal handler
        static::$globalEvent->add(SIGTERM, EventInterface::EV_SIGNAL, array(
            '\Workerman\Worker',
            'signalHandler'
        ));
        // reinstall reload signal handler
        static::$globalEvent->add(SIGUSR1, EventInterface::EV_SIGNAL, array(
            '\Workerman\Worker',
            'signalHandler'
        ));
        // reinstall graceful reload signal handler
        static::$globalEvent->add(SIGQUIT, EventInterface::EV_SIGNAL, array(
            '\Workerman\Worker',
            'signalHandler'
        ));
        // reinstall status signal handler
        static::$globalEvent->add(SIGUSR2, EventInterface::EV_SIGNAL, array(
            '\Workerman\Worker',
            'signalHandler'
        ));
        // reinstall connection status signal handler
        static::$globalEvent->add(SIGIO, EventInterface::EV_SIGNAL, array(
            '\Workerman\Worker',
            'signalHandler'
        ));
    }
    ...
    public static function forkOneWorkerForWindows($start_file)
    {
        ...
        $std_file = sys_get_temp_dir() . '/' . str_replace(array(
            '/',
            "\\",
            ':'
    ...
    protected static function reload()
    {
        ...
        if (static::$_masterPid === posix_getpid()) {
            ...
            if (! static::$_gracefulStop) {
                Timer::add(static::KILL_WORKER_TIMER_TIME, 'posix_kill', array(
                    $one_worker_pid,
                    SIGKILL
                ), false);
            }
    ...
    protected static function writeStatisticsToStatusFile()
    {
        ...
        if (static::$_masterPid === posix_getpid()) {
            ...
            $loadavg = function_exists('sys_getloadavg') ? array_map('round', sys_getloadavg(), array(
                2
            )) : array(
                '-',
                '-',
    ...
    public static function log($msg)
    {
        ...
        file_put_contents((string) static::$logFile, date('Y-m-d H:i:s') . ' ' . 'pid:' . (static::$_OS === 'linux' ? posix_getpid() : 1) . ' ' . $msg, FILE_APPEND | LOCK_EX);
    }
    ...
    public function resumeAccept()
    {
        ...
        if (static::$globalEvent && true === $this->_pauseAccept && $this->_mainSocket) {
            if ($this->transport !== 'udp') {
                static::$globalEvent->add($this->_mainSocket, EventInterface::EV_READ, array(
                    $this,
                    'acceptConnection'
                ));
            } else {

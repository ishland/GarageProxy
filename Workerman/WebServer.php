...
class WebServer extends Worker
{
    ...
    public function onWorkerStart ()
    {
        if (empty($this->serverRoot)) {
            Worker::safeEcho(
                    new \Exception(
                            'server root not set, please use WebServer::addRoot($domain, $root_path) to set server root path'));
            exit(250);
        }
    ...
    public function initMimeTypeMap ()
    {
        ...
        foreach ($items as $content) {
            if (preg_match("/\s*(\S+)\s+(\S.+)/", $content, $match)) {
                ...
                $workerman_file_extension_array = explode(' ',
                        substr($workerman_file_extension_var, 0, - 1));
    ...
    public function onMessage ($connection)
    {
        ...
        if ($workerman_file_extension === '') {
            $workerman_path = ($len = strlen($workerman_path)) &&
                    $workerman_path[$len - 1] === '/' ? $workerman_path .
                    'index.php' : $workerman_path . '/index.php';
            $workerman_file_extension = 'php';
        }
        
        $workerman_siteConfig = isset(
                $this->serverRoot[$_SERVER['SERVER_NAME']]) ? $this->serverRoot[$_SERVER['SERVER_NAME']] : current(
                $this->serverRoot);
        ...
        if (is_file($workerman_file)) {
            ...
            if (isset($workerman_siteConfig['custom404']) &&
                    file_exists($workerman_siteConfig['custom404'])) {
    ...
    public static function sendFile ($connection, $file_path)
    {
        // Check 304.
        $info = stat($file_path);
        $modified_time = $info ? date('D, d M Y H:i:s', $info['mtime']) . ' ' .
        ...
        if ($file_size < $trunk_limit_size) {
            return $connection->send($header . file_get_contents($file_path),
                    true);
        }

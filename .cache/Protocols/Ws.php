...
class Ws
{
    ...
    public static function input ($buffer, $connection)
    {
        ...
        if ($connection->websocketCurrentFrameLength) {
            ...
            switch ($opcode) {
                ...
                case 0x8:
                    ...
                    if (isset($connection->onWebSocketClose)) {
                        try {
                            call_user_func($connection->onWebSocketClose,
                                    $connection);
                        } catch (\Exception $e) {
                            Worker::log($e);
                            exit(250);
                ...
                case 0x9:
                    ...
                    if (isset($connection->onWebSocketPing)) {
                        try {
                            call_user_func($connection->onWebSocketPing,
                                    $connection);
                        } catch (\Exception $e) {
                            Worker::log($e);
                            exit(250);
                    ...
                    if (! $data_len) {
                        ...
                        if ($recv_len > $head_len) {
                            return self::input(substr($buffer, $head_len),
                                    $connection);
                        }
                        return 0;
                    }
                ...
                case 0xa:
                    ...
                    if (isset($connection->onWebSocketPong)) {
                        try {
                            call_user_func($connection->onWebSocketPong,
                                    $connection);
                        } catch (\Exception $e) {
                            Worker::log($e);
                            exit(250);
            ...
            if ($data_len === 126) {
                ...
                if ($data_len === 127) {
                    ...
                    $current_frame_length = $arr['c1'] * 4294967296 + $arr['c2'] +
                            10;
                } else {
        ...
        elseif ($connection->websocketCurrentFrameLength < $recv_len) {
            ...
            return self::input(substr($buffer, $current_frame_length),
                    $connection);
    ...
    public static function encode ($payload, $connection)
    {
        ...
        if (65535 < $length) {
            $pack = pack('NN', ($length & 0xFFFFFFFF00000000) >> 32,
        ...
        if ($connection->handshakeStep === 1) {
            ...
            $connection->tmpWebsocketData = $connection->tmpWebsocketData .
                    $frame;
    ...
    public static function sendHandshake ($connection)
    {
        ...
        $host = $port === 80 ? $connection->getRemoteHost() : $connection->getRemoteHost() .
                ':' . $port;
        // Handshake header.
        ...
        $header = 'GET ' . $connection->getRemoteURI() . " HTTP/1.1\r\n" .
                "Host: $host\r\n" . "Connection: Upgrade\r\n" .
                "Upgrade: websocket\r\n" . "Origin: " .
                (isset($connection->websocketOrigin) ? $connection->websocketOrigin : '*') .
                "\r\n" .
                (isset($connection->WSClientProtocol) ? "Sec-WebSocket-Protocol: " .
                $connection->WSClientProtocol . "\r\n" : '') .
                "Sec-WebSocket-Version: 13\r\n" . "Sec-WebSocket-Key: " .
                $connection->websocketSecKey . $userHeader . "\r\n\r\n";
    ...
    public static function dealHandshake ($buffer, $connection)
    {
        ...
        if ($pos) {
            ...
            if (strlen($buffer) > $handshake_response_length) {
                return self::input(substr($buffer, $handshake_response_length),
                        $connection);
            }
        }
        return 0;

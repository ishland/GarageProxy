...
class Websocket implements \Workerman\Protocols\ProtocolInterface
{
    ...
    public static function input ($buffer, ConnectionInterface $connection)
    {
        ...
        if ($connection->websocketCurrentFrameLength) {
            ...
            if ($connection->websocketCurrentFrameLength > $recv_len) {
                // Return 0, because it is not clear the full packet length,
                // waiting for the frame of fin=1.
                return 0;
            }
        } else {
            ...
            switch ($opcode) {
                ...
                case 0x8:
                    ...
                    if (isset($connection->onWebSocketClose) ||
                            isset($connection->worker->onWebSocketClose)) {
                        try {
                            call_user_func(
                                    isset($connection->onWebSocketClose) ? $connection->onWebSocketClose : $connection->worker->onWebSocketClose,
                                    $connection);
                        } catch (\Exception $e) {
                            Worker::log($e);
                            exit(250);
                ...
                case 0x9:
                    ...
                    if (isset($connection->onWebSocketPing) ||
                            isset($connection->worker->onWebSocketPing)) {
                        try {
                            call_user_func(
                                    isset($connection->onWebSocketPing) ? $connection->onWebSocketPing : $connection->worker->onWebSocketPing,
                                    $connection);
                        } catch (\Exception $e) {
                            Worker::log($e);
                            exit(250);
                    ...
                    if (! $data_len) {
                        ...
                        if ($recv_len > $head_len) {
                            return static::input(substr($buffer, $head_len),
                                    $connection);
                        }
                        return 0;
                    }
                ...
                case 0xa:
                    ...
                    if (isset($connection->onWebSocketPong) ||
                            isset($connection->worker->onWebSocketPong)) {
                        try {
                            call_user_func(
                                    isset($connection->onWebSocketPong) ? $connection->onWebSocketPong : $connection->worker->onWebSocketPong,
                                    $connection);
                        } catch (\Exception $e) {
                            Worker::log($e);
                            exit(250);
                ...
                // Wrong opcode.
                default:
                    echo "error opcode $opcode and close websocket connection. Buffer:" .
                            bin2hex($buffer) . "\n";
                    $connection->close();
                    return 0;
            }
        ...
        if ($connection->websocketCurrentFrameLength === $recv_len) {
            ...
            return 0;
        } // The length of the received data is greater than the length of a
          // frame.
        elseif ($connection->websocketCurrentFrameLength < $recv_len) {
            ...
            return static::input(substr($buffer, $current_frame_length),
                    $connection);
    ...
    public static function encode ($buffer, ConnectionInterface $connection)
    {
        ...
        if ($len <= 125) {
            ...
            if ($len <= 65535) {
                $encode_buffer = $first_byte . chr(126) . pack("n", $len) .
                        $buffer;
            } else {
                $encode_buffer = $first_byte . chr(127) . pack("xxxxN", $len) .
                        $buffer;
            }
        }
        
        // Handshake not completed so temporary buffer websocket data waiting
        // for send.
        if (empty($connection->websocketHandshake)) {
            ...
            if (strlen($connection->tmpWebsocketData) >
                    $connection->maxSendBufferSize) {
                if ($connection->onError) {
                    try {
                        call_user_func($connection->onError, $connection,
                                WORKERMAN_SEND_FAIL,
    ...
    protected static function dealHandshake ($buffer, $connection)
    {
        ...
        if (0 === strpos($buffer, 'GET')) {
            ...
            if (isset($connection->onWebSocketConnect) ||
                    isset($connection->worker->onWebSocketConnect)) {
                static::parseHttpHeader($buffer);
                try {
                    call_user_func(
                            isset($connection->onWebSocketConnect) ? $connection->onWebSocketConnect : $connection->worker->onWebSocketConnect,
                            $connection, $buffer);
                } catch (\Exception $e) {
                    Worker::log($e);
                    exit(250);
                ...
                if (! empty($_SESSION) &&
                        class_exists('\GatewayWorker\Lib\Context')) {
                    $connection->session = \GatewayWorker\Lib\Context::sessionEncode(
                            $_SESSION);
                }
